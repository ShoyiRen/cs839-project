...
matcher = result['selected_matcher']

# Evaluate the matcher
L = em.extract_feature_vecs(J, feature_table=F, attrs_after=['gold_labels'])
L.fillna(value='NaN', inplace=True)
matcher.fit(table=H, exclude_attrs=['_id', 'ltable_ID', 'rtable_ID', 'gold_labels'], target_attr='gold_labels')
predictions = matcher.predict(table=L, exclude_attrs=['_id', 'ltable_ID', 'rtable_ID', 'gold_labels'], target_attr='predicted_labels', append = True)
